...
// único, sempre igual, cartão a cartão. O cartão ativo também ganha um
// leve destaque (escala e opacidade, via `activeIndex`) sobre os vizinhos,
// o mesmo princípio de profundidade que já diferencia a fatia em foco na
// pilha de desktop (ver coveredScale/coveredDim em SlidePanel), só que aqui
// a MUDANÇA de foco vem do gesto de arrastar, não do scroll vertical da
// página: arrastar É a navegação, sem seta, sem autoplay, o mesmo princípio
// de "rolar é o controle" que já rege a versão de desktop, só que no eixo
// horizontal. Os pontos abaixo do trilho leem a posição (não navegam, mesmo
        ...
        {cases.map((caseStudy, index) => {
          ...
          return (
            <Link
              ...
              href={`/${locale}/work/${caseStudy.slug}/`}
              className={`relative block aspect-[3/4] w-[82vw] shrink-0 snap-start overflow-hidden bg-black text-white transition-[transform,opacity] duration-500 ease-out ${
