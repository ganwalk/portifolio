...
/** Muda qualquer áudio do documento pra silencioso, de vez, cobrindo tanto
...
function silenceAudio(doc: Document) {
  ...
  const mute = (el: HTMLMediaElement) => {
    el.muted = true;
    el.volume = 0;
  };

  doc.querySelectorAll("audio, video").forEach((el) => mute(el as HTMLMediaElement));
  ...
  // público que o override abaixo intercepta): muta no instante em que o
  // elemento aparece, antes do navegador ter chance de tocar alguma coisa.
  new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      mutation.addedNodes.forEach((node) => {
        if (!(node instanceof win.Element)) return;
        if (node instanceof win.HTMLMediaElement) mute(node);
        node.querySelectorAll?.("audio, video").forEach((el) => mute(el as HTMLMediaElement));
      });
    }
  ...
  // coisa): silencia no instante em que o próprio site tenta iniciar a
  // reprodução, antes do primeiro quadro de áudio sair.
